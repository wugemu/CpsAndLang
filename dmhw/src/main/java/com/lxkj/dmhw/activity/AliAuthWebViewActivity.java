...
//跳外链（主要是支持天猫超市 网页查找优惠券）
public class AliAuthWebViewActivity extends BaseActivity implements AlibcTradeCallback {
    ...
    private void doInit() {
        ...
        web.addJavascriptInterface(new JavascriptHandler(this,4), "live");
        web.post(() -> {
            if (noGoTaoBaoH5==12) {
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenu.ContextMenuInfo info) {
        ...
        MenuItem.OnMenuItemClickListener listener = item -> {
            ...
            switch (item.getTitle().toString()) {
                case "保存图片":
                    isCheck = true;
                    if (Utils.checkPermision(this,Utils.WRITE_EXTERNAL_STORAGE_REQUEST,true)) {
                        Glide.with(this).asBitmap()
                            ...
                            public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
                                ...
                                ToastUtil.showImageToast(AliAuthWebViewActivity.this, "图片已保存至相册", R.mipmap.toast_img);
                            }
                        });
                    }
                    break;
...
//            webProgress.setProgress(newProgress);
    ...
    private WebViewClient webViewClient = new WebViewClient() {
        ...
        public boolean shouldOverrideUrlLoading(WebView view, String url) {
            try {
                    if (url.startsWith("pinduoduo://")) {
                        boolean hasInstalled = Utils.checkHasInstalledApp(AliAuthWebViewActivity.this, "com.xunmeng.pinduoduo");
                        if (hasInstalled){
                            Intent intent=new Intent(Intent.ACTION_VIEW, Uri.parse(url));
                            startActivity(intent);
                        }else{
                            ToastUtil.showImageToast(AliAuthWebViewActivity.this,"未安装拼多多App",R.mipmap.toast_img);
                        }
                        return true;
                    }
                    if (url.startsWith("vipshop://")){
                        boolean hasInstalled = Utils.checkHasInstalledApp(AliAuthWebViewActivity.this, "com.achievo.vipshop");
                        if (hasInstalled) {
                            Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(url));
                            startActivity(intent);
                        }
                    ...
                    if (url.startsWith("hap://app/com.VIP.VIPQuickAPP")){
                ...
                if (url.startsWith("weixin://")) {
                    try {
                        ...
                        Toast.makeText(AliAuthWebViewActivity.this, "请安装微信App", Toast.LENGTH_LONG).show();
                    }
                    return true;
                }
                if (url.startsWith("https://wx.tenpay.com")) {
                    ...
                    if (url.startsWith("openapp.jdmobile://")){
                        boolean hasInstalled = Utils.checkHasInstalledApp(AliAuthWebViewActivity.this, "com.jingdong.app.mall");
                        if (hasInstalled){
                            Intent intent=new Intent(Intent.ACTION_VIEW, Uri.parse(url));
                            startActivity(intent);
                        }else{
                            ToastUtil.showImageToast(AliAuthWebViewActivity.this,"未安装京东App",R.mipmap.toast_img);
                        }
                        return true;
                    }
                ...
                if (url.toLowerCase().startsWith("taobao://") && isGoTaobao) {
                    Utils.Alibc(AliAuthWebViewActivity.this, url.replace("taobao://", "https://"), null, null);
                    ...
                    if (!url.toLowerCase().startsWith("alipays://")) {
                        if (url.toLowerCase().startsWith("http://") || url.toLowerCase().startsWith("https://")) {
                             ...
                             */
                            final PayTask task = new PayTask(AliAuthWebViewActivity.this);
                            boolean isIntercepted = task.payInterceptorWithUrl(url, true, new H5PayCallback() {
                                @Override
                                public void onPayResult(final H5PayResultModel result) {
                                    AliAuthWebViewActivity.this.runOnUiThread(new Runnable() {
                                        @Override
                                        public void run() {
                ...
                //拦截打开非网页链接
                if (!url.startsWith("tbopen://")&&!url.startsWith("http://")&&!url.startsWith("https://")&&!url.startsWith("taobao://")){
                    try {
                    } catch (Exception e) {
                        Toast.makeText(AliAuthWebViewActivity.this, "未安装该应用", Toast.LENGTH_LONG).show();
                    }
                    return true;
                }
            }catch (Exception e){
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            if (NetStateUtils.isNetworkConnected(AliAuthWebViewActivity.this)) {
...
//        }
    ...
    public void handlerMessage(Message message) {
        if (message.what == LogicActions.GetRatioSuccess) {
            ...
            if(noGoTaoBaoH5==12){
                startActivity(new Intent(this, CommodityActivity290.class).putExtra("shopId", searchAll.getNumIid()).putExtra("source", "tb").putExtra("sourceId",""));
            }else{
        ...
        if (message.what == LogicActions.GetCommissionSuccess) {
            ...
            if (shareList.getTips().getKey().equals("")){
                ...
                if (shareList.getTips().getKey().equals(DateStorage.getIsKey()) || isshowDialog) {
                    ...
                    try{
                        CouponLinkDialog couponLinkDialog = new CouponLinkDialog(AliAuthWebViewActivity.this,shareList.getTips());
                    ...
                    try{
                        ...
                        couponLinkDialog.setOnClickListener(new CouponLinkDialog.OnBtnClickListener() {
                            ...
                            public void onClick(int pos) {
                                ...
                                dialog.showDialog();
                                Utils.Alibc(AliAuthWebViewActivity.this, Couponlink, pid, AliAuthWebViewActivity.this);
                                new Handler().postDelayed(dialog::hideDialog, 3500);
                            }
                        });
                    }catch(Exception e){
        ...
        if (message.what == LogicActions.ConvertShopidSuccess) {
            dismissDialog();
            try {
                JSONObject object=(JSONObject) message.obj;
                String type=object.getString("cpsType");
                String goodsid=object.getString("goodsId");
                switch (type){
                    case "tb":
                    case "tm":
                        startActivity(new Intent(this, CommodityActivity290.class).putExtra("shopId", goodsid).putExtra("source", "tb").putExtra("sourceId",""));
                        break;
                    case "pdd":
                        startActivity(new Intent(this, CommodityActivityPJW.class).putExtra("GoodsId", goodsid).putExtra("type", "pdd"));
                        break;
                    case "jd":
                        startActivity(new Intent(this, CommodityActivityPJW.class).putExtra("GoodsId", goodsid).putExtra("type", "jd"));
                        break;
                    case "wph":
                        startActivity(new Intent(this, CommodityActivityPJW.class).putExtra("GoodsId", goodsid).putExtra("type", "wph"));
                        break;
                    case "sn":
                        startActivity(new Intent(this, CommodityActivityPJW.class).putExtra("GoodsId", goodsid).putExtra("type", "sn"));
                        break;
                    case "kl":
                        startActivity(new Intent(this, CommodityActivityPJW.class).putExtra("GoodsId", goodsid).putExtra("type", "kl"));
                        break;
                }
    ...
    public  class SaveImage extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) {
            ...
            try {
                ...
                file = new File(Variable.picturePath + "/", "二维码图片" + Utils.getCurrentDate("yyyyMMdd") + Utils.getRandom(0, 9) + Utils.getRandom(0, 9) + Utils.getRandom(0, 9) + Utils.getRandom(0, 9) + Utils.getRandom(0, 9) + Utils.getRandom(0, 9) + ".jpg");
    ...
    public  void  downLoad(String url){
        imgurl=url;
        new AliAuthWebViewActivity.SaveImage().execute();
    }
    ...
    public void showProgressDialog(String title, String message) {
        if (progressDialog == null) {
            progressDialog = ProgressDialog.show(AliAuthWebViewActivity.this, title,
                    message, true, false);
        ...
        if (!Variable.encrypt.equals("")) {
            KDFInterface.getInstance().login(this,Variable.encrypt, bonusRate, showType, isShare, shareRate, new OnComplete() {
                ...
                public void onSuccess(JSONObject o) {
                    if (type==0){
                        KDFInterface.getInstance().toCardStoreActivity(AliAuthWebViewActivity.this, null);
                    }else {
                        try{
                        //查询办卡进度
                        KDFInterface.getInstance().toOrderProgressActivity(AliAuthWebViewActivity.this, tempJson.getString("bankCode"), tempJson.getString("id"), tempJson.getString("custName"), tempJson.has("crawId") ? tempJson.getString("crawId") : "", null);
                        }catch (Exception e){
                        }
                        }
                }
