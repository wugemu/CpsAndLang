package com.lxkj.dmhw.activity;

import android.os.Build;
import android.os.Bundle;
import android.os.Message;
import androidx.recyclerview.widget.RecyclerView;
import android.view.View;
import android.widget.LinearLayout;
import android.widget.TextView;

import com.chad.library.adapter.base.BaseQuickAdapter;
import com.lxkj.dmhw.Variable;
import com.lxkj.dmhw.defined.BaseActivity;
import com.lxkj.dmhw.R;
import com.lxkj.dmhw.adapter.ScoreDetailsAdapter;
import com.lxkj.dmhw.bean.ScoreDetails;
import com.lxkj.dmhw.logic.HttpCommon;
import com.lxkj.dmhw.logic.LogicActions;
import com.lxkj.dmhw.logic.NetworkRequest;
import com.lxkj.dmhw.utils.MyLayoutManager;

import java.util.HashMap;

import butterknife.BindView;
import butterknife.ButterKnife;
import butterknife.OnClick;

public class ScoreDetailsActivity extends BaseActivity implements BaseQuickAdapter.RequestLoadMoreListener {

    @BindView(R.id.bar)
    View bar;
    @BindView(R.id.back)
    LinearLayout back;
    @BindView(R.id.mTitle)
    TextView mTitle;
    @BindView(R.id.score_details_recycler)
    RecyclerView scoreDetailsRecycler;
    private ScoreDetailsAdapter adapter;
    private String type;
    private String time;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_score_details);
        ButterKnife.bind(this);
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            bar.setVisibility(View.GONE);
        }
        if (Variable.statusBarHeight>0) {
            LinearLayout.LayoutParams linearParams = (LinearLayout.LayoutParams) bar.getLayoutParams(); //取控件textView当前的布局参数 linearParams.height = 20;// 控件的高强制设成20
            linearParams.height = Variable.statusBarHeight;
            bar.setLayoutParams(linearParams); //使设置好的布局参数应用到控件
        }
        type = getIntent().getExtras().getString("type");
        switch (type) {
            case "00":
                mTitle.setText("心得明细");
                break;
            case "01":
                mTitle.setText("点赞明细");
                break;
            case "02":
                mTitle.setText("转发明细");
                break;
            case "03":
                mTitle.setText("签到明细");
                break;
            case "04":
                mTitle.setText("兑换明细");
                break;
        }
        // 设置Recycler
        scoreDetailsRecycler.setLayoutManager(MyLayoutManager.getInstance().LayoutManager(this, false));
        adapter = new ScoreDetailsAdapter(type);
        scoreDetailsRecycler.setAdapter(adapter);
        // 设置预加载位置,倒数
        adapter.setPreLoadNumber(5);
        // 监听加载更多事件
        adapter.setOnLoadMoreListener(this, scoreDetailsRecycler);
        // 取消第一次加载回调
        adapter.disableLoadMoreIfNotFullPage();
        httpPost("");
    }

    @Override
    public void mainMessage(Message message) {

    }

    @Override
    public void childMessage(Message message) {

    }

    @Override
    public void handlerMessage(Message message) {
        if (message.what == LogicActions.ScoreDetailsSuccess) {
            ScoreDetails details = (ScoreDetails) message.obj;
            time = details.getSearchtime();
            if (details.getScoredetail().size() > 0) {
                if (page > 1) {
                    adapter.addData(details.getScoredetail());
                    adapter.notifyDataSetChanged();
                } else {
                    adapter.setNewData(details.getScoredetail());
                    adapter.notifyDataSetChanged();
                }
                adapter.loadMoreComplete();
            } else {

                adapter.loadMoreEnd();
            }
        }
    }

    @OnClick(R.id.back)
    public void onViewClicked() {
        isFinish();
    }

    @Override
    public void onLoadMoreRequested() {
        page++;
        httpPost(time);
    }

    private void httpPost(String time) {
        paramMap = new HashMap<>();
        paramMap.put("userid", login.getUserid());
        paramMap.put("stype", type);
        paramMap.put("startindex", page + "");
        paramMap.put("searchtime", time);
        paramMap.put("pagesize", pageSize + "");
        NetworkRequest.getInstance().POST(handler, paramMap, "ScoreDetails", HttpCommon.ScoreDetails);
    }
}
