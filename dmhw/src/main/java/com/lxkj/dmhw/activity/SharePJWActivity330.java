...
public class SharePJWActivity330 extends BaseActivity implements View.OnTouchListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        shareInfo.setType(getIntent().getExtras().getString("type"));
        ...
        shareRecycler.addItemDecoration(new LinearItemDecoration(Utils.dipToPixel(R.dimen.dp_13), 0, 0, 0));
        adapter = new ShareAdapter330(this);
        shareRecycler.setAdapter(adapter);
        if(shareInfo.getDiscount().equals("")){
           shareInfo.setDiscount("0");
        }
        try{
        ...
        if(getIntent().getStringExtra("type").equals("wph")||getIntent().getStringExtra("type").equals("kl")){
            duanlianjie = TextAndLinkWPH.replace("{短链接}", shareInfo.getShortLink());
            if(shareInfo.getRecommended()==null||shareInfo.getRecommended().equals("")){
                erweima = OnlyTextWPH.replace("{标题}", shareInfo.getName()).replace("{原价}", price).replace("{券后价}", shareInfo.getMoney()).replace("{推荐内容}","");
            }else {
                erweima = OnlyTextWPH.replace("{标题}", shareInfo.getName()).replace("{原价}", price).replace("{券后价}", shareInfo.getMoney()).replace("{推荐内容}", shareInfo.getRecommended());
            }
            }else {
            duanlianjie = TextAndLink.replace("{短链接}", shareInfo.getShortLink());
            erweima = OnlyText.replace("{标题}", shareInfo.getName()).replace("{原价}", price).replace("{券后价}", shareInfo.getMoney()).replace("{券额}", shareInfo.getDiscount()).replace("{推荐内容}", shareInfo.getRecommended());
        }
        }catch (Exception e){

        }
        radioGroupChange(DateStorage.getShareTextTypePJW());//仅文案
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.share_erweima_text:
                shareChangeMainImageDialog.showShareMainImageDialog(shareInfo,mainImagePos,"pjw");
                break;
                //批量存图
                    ...
                    for (int i = 0; i < images.size(); i++) {
                        ...
                        Glide.with(this).asBitmap().load(images.get(finalI)).into(new SimpleTarget<Bitmap>(){
                            ...
                            public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
                                if (finalI==0){
                                            Utils.saveFile(Variable.imagesPath, Utils.jointBitmapPJW(SharePJWActivity330.this, resource,params.getShareInfo().get(finalI)), 100, true);
                                        }else{
...
//    @Override
//    public void onShareClick(int num,int pos,ShareCheck item) {
//        number = num;
//        shareCheckText.setText("已选" + number + "张");
//    }
    ...
    private void radioGroupChange(int textType){
        ...
        DateStorage.setShareTextTypePJW(textType);
