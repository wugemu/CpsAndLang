...
public class MorePlFragment340 extends LazyFragment implements BaseQuickAdapter.RequestLoadMoreListener{
    ...
    public static MorePlFragment340 getInstance(String  capId, int posNum, int plattype) {
        MorePlFragment340 fragment = new MorePlFragment340();
    ...
    public void onEvent() {
        ...
        image.setImageResource(R.mipmap.pjw_no_goods);
        textView1.setText("此分类暂无商品");
...
//        // 设置预加载位置,倒数
    ...
    public void onResume() {
        super.onResume();
        if (loginRefreshFlag&&posNum==MorePlatformNewActivity340.currentPos){
            ...
            if (jdGoodsBeanArrayList.size() > 0) {
                if (jdGoodsBeanArrayList.size() < pageSize) {
                    if (page > 1) {
                        ...
                        ((MorePlatformNewActivity340) getActivity()).refreshComplete();
                    }
                    adapter.loadMoreEnd();
                }else{
                    if (page > 1) {
