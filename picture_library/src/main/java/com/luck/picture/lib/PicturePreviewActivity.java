        ...
        ll_check.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (images != null && images.size() > 0) {
                    ...
                    if (!TextUtils.isEmpty(pictureType)&&!check.isSelected()) {
                        boolean toEqual = PictureMimeType.
                                mimeToEqual(pictureType, image.getPictureType());
                        if (isShortVideo()&&selectImages.size()>2) {
                            ToastManage.s(mContext, mContext.getString(R.string.picture_rule2));
                            return;
                        }
                        if (!toEqual&&!isShortVideo()) {
                    ...
                    if(eqImg) {
                        ...
                        //选中的视频
                        if (selectImages.size() >= 1&&!isShortVideo() && isChecked) {
                            String str = mContext.getString(R.string.picture_message_video_max_num, 1);
...
//                        //视频大于十秒
//                        ToastManage.s(mContext, "视频时长限十秒");
//                        check.setSelected(false);
//                        return;
//                    }
    ...
    private boolean isShortVideo(){
        if (PictureMimeType.isPictureType(selectImages.get(0).getPictureType())==PictureConfig.TYPE_VIDEO&&selectImages.size()<=2){
            return true;
        }
        return false;
    }
